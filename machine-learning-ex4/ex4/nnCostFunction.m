...
d = ( -ymat.*log(a3) )- ( (1-ymat).*log(1-a3) );
d = d * ones(num_labels,1);
J = d' * ones(m,1);
J = J/m;
reg1 = ( Theta1(:, 2:end).^2 * ones(size(Theta1(:, 2:end),2),1) )' * ones(size(Theta1(:, 2:end),1),1);
reg2 = ( Theta2(:, 2:end).^2 * ones(size(Theta2(:, 2:end),2),1) )' * ones(size(Theta2(:, 2:end),1),1);
